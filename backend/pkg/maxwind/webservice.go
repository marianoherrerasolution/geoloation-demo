...
func City(ipAddress string) (GeoIPInfo, error) {
	webservice := New(config.Env.MAXMIND_ACCOUNT_ID, config.Env.MAXMIND_LICENSE_KEY, "geoip.maxmind.com")
...
func Country(ipAddress string) (GeoIPInfo, error) {
	webservice := New(config.Env.MAXMIND_ACCOUNT_ID, config.Env.MAXMIND_LICENSE_KEY, "geolite.info")
	return webservice.request("/geoip/v2.1/city/", ipAddress)
}
